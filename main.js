...
const chargeUpFunc = {
	...
	init: function () {
		...
		if (purchaseItemData.get(config1.chargeUp.purchaseItem[0]) === undefined) {
			for (let i = 0; i < config1.chargeUp.purchaseItem.length; i++) {
				purchaseItemData.put(config1.chargeUp.purchaseItem[i], 0);
				if (i >= config1.chargeUp.paymentSoftWare.length) continue;
				pcswItemData.put(config1.chargeUp.paymentSoftWare[i], 0);
				if (i >= config1.chargeUp.incomeItem.length) continue;
				incomeItemData.put(config1.chargeUp.incomeItem[i], 0);
			}
		}
		...
		for (let i = 0; i < config1.chargeUp.purchaseItem.length; i++) {
			purchaseItemListNum = this.sortItem(
				purchaseItemListNum,
				config1.chargeUp.purchaseItem[i],
				purchaseItemData.get(config1.chargeUp.purchaseItem[i])
			);
			if (i >= config1.chargeUp.paymentSoftWare.length) continue;
			pcswItemListNum = this.sortItem(
				pcswItemListNum,
				config1.chargeUp.paymentSoftWare[i],
				pcswItemData.get(config1.chargeUp.paymentSoftWare[i])
			);
			if (i >= config1.chargeUp.incomeItem.length) continue;
			incomeItemListNum = this.sortItem(
				incomeItemListNum,
				config1.chargeUp.incomeItem[i],
				incomeItemData.get(config1.chargeUp.incomeItem[i])
			);
		}
		for (let i = 0; i < config1.chargeUp.purchaseItem.length; i++) {
	...
	autoGetCate: function (PI) {
		...
		let textArr = [];
		let PItype = PI === 0 ? config1.chargeUp.purchaseItem.join(",") : config1.chargeUp.incomeItem.join(",");
		...
		let postData = {
			model: config1.model.model,
			messages: [
				{
					role: "system",
					content: config1.prompt.replace("[[replace]]", PItype),
				},
				{
					role: "user",
		...
		if (config1.modelSelected == 0) {
			postData.tools = [{ type: "web_search", web_search: { enable: false } }];
		}
		var res;
		var thread = threads.start(function () {
			res = http.postJson(config1.model.url, postData, {
				headers: {
					"Content-Type": "application/json",
					Authorization: config1.model.authorization(),
				},
			});
		});
		thread.join(config1.overtime);
		thread.interrupt();
		...
		let items = PI === 0 ? config1.chargeUp.purchaseItem : config1.chargeUp.incomeItem;
		for (let i in items) {
			if (items[i] === response) {
				return response;
			}
...
function startChargeUp(packagename, compareTextReg, amountReg, timeReg, defaultCate) {
	...
	let pcswReg = new RegExp(
		"(" +
			Object.keys(config1.chargeUp.mappingPcsw)
				.join("|")
				.replace(/\(/g, "\\(")
				.replace(/\)/g, "\\)(\\s+)?")
				.replace(/\*/g, "\\*") +
	...
	let accountList = textMatches(pcswReg).exists()
		? textMatches(pcswReg).findOne().text().replace(" ", "")
		: defaultCate;
	let accountGet = config1.chargeUp.mappingPcsw[accountList];
	chargeUpFunc.init();
	toastLog("获取类目中……");
...
function initialization() {
	sleep(100);
	for (let i = 0; i < config1.chargeUp.platform.length; i++) {
		checkReapet.put(config1.chargeUp.platform[i]["id"], 0);
		let packageName = config1.chargeUp.platform[i].packageName,
			compareTextReg = config1.chargeUp.platform[i].compareTextReg,
			amountReg = config1.chargeUp.platform[i].amountReg,
			timeReg = config1.chargeUp.platform[i].timeReg,
			defaultCate = config1.chargeUp.platform[i].defaultCate;
		if (currentPackage() == packageName) {
			startChargeUp(packageName, compareTextReg, amountReg, timeReg, defaultCate);
			break;
		} else if (i === config1.chargeUp.platform.length - 1) {
			alert("未在支付软件界面", currentPackage());
			exit();
		}
	}
}
